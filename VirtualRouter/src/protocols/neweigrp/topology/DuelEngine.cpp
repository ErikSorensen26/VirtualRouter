...
std::optional<RouteInfo> DuelEngine::findBestRoute(const IPPrefix& prefix)
{
    auto* entry = topologyTable.find(prefix);
    std::lock_guard<std::mutex> lock(topologyTable.tableMutex);
    if (!entry || entry->routesByNeighbor.empty() || entry->successors.empty()) return std::nullopt;

    return entry->routesByNeighbor[entry->successors.front()].routeInfo;
}
...
void DuelEngine::updateSuccessors(std::vector<TopologyEntry*>& entries, const IPAddress& updatedNeighbor)
{
    ...
    for (auto& entry : entries)
    {
        ...
        auto& route = entry->routesByNeighbor[updatedNeighbor];
...
void DuelEngine::setActive(const std::vector<IPPrefix>& prefixes, const IPAddress& failedNeighbor, const uint32_t* seq)
{
    ...
    base.routeManager.withdrawRoute(prefixes);
