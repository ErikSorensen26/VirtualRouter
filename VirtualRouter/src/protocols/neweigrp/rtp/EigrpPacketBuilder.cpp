...
size_t EigrpPacketBuilder::appendRoutes(TLV16BufferManager& tlv, const std::vector<const RouteInfo*>& routes, uint64_t bw, uint64_t delay, TLVType tlvVersion)
{
    ...
    for (auto* r : routes)
    {
        ...
        TLVBuilder::RouteType type = r->routeInfo.routeType == RoutingTable::Eigrp::RouteType::EXTERNAL
...
size_t EigrpPacketBuilder::appendQueries(TLV16BufferManager& tlv, const std::vector<OutgoingQuery*>& queries, uint32_t seq, uint32_t bw, uint32_t delay, TLVType tlvVersion)
{
    size_t appended = 0;
    for (auto* q : queries)
    {
        auto* buf = tlv.getNextValBuf();
        TLVBuilder::RouteType type = q->route.originRoute->routeInfo.routeType == RoutingTable::Eigrp::RouteType::EXTERNAL
            ? static_cast<TLVBuilder::RouteType>(static_cast<uint8_t>(tlvVersion) ^ 1)
            : static_cast<TLVBuilder::RouteType>(tlvVersion);
...
size_t EigrpPacketBuilder::appendSIAQueries(TLV16BufferManager& tlv, const std::vector<OutgoingQuery*>& queries, uint32_t seq, uint32_t bw, uint32_t delay, TLVType tlvVersion)
{
    size_t appended = 0;
    for (auto* q : queries)
    {
