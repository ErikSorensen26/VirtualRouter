...
public:
    ...
    static size_t appendQueries(TLV16BufferManager& tlv, const std::vector<OutgoingQuery*>& queries, uint32_t seq, uint32_t bw, uint32_t delay, TLVType tlvVersion);
    static size_t appendSIAQueries(TLV16BufferManager& tlv, const std::vector<OutgoingQuery*>& queries, uint32_t seq, uint32_t bw, uint32_t delay, TLVType tlvVersion);
};
}
