...
protected:
    ...
    struct HeaderInfo
    {
        HeaderInfo(uint8_t* pload, size_t psize, uint16_t len, const IPAddress& neighborIp, const uint32_t rid)
            : rid(rid), packetSize(psize), payloadSize(len), payload(pload), neighborIp(neighborIp) {}
