			...
			else if (commandStream[0] == "ip")
			{
				if (commandStream[1] == "dhcp")
				{
					...
					else if (commandStream[2] == "binding")
					{
						...
						Global::getInstance().dhcpServer->globalConfig.bindingCleanup.store(cleanupTime, std::memory_order_release);
					}
		...
		if (currentMode == mode.routing || currentMode == mode.addressFamily || currentMode == mode.addressFamilyTopology || currentMode == mode.routingV6)
		{
			if (currentSubMode == "eigrp_classic" || currentSubMode == "eigrp_classic_vrf" || (currentSubMode == "eigrp_named" && currentMode == mode.addressFamilyTopology))
			{
				...
				else if (commandStream[0] == "eigrp")
				{
					...
					else if (commandStream[1] == "logNeighborChanges") { currentEigrp->configs.logNeighborChanges = !no; }
					else if (commandStream[1] == "logNeighborWarnings") { currentEigrp->configs.logNeighborWarnings = !no; }
				...
				else if (commandStream[0] == "metric")
				{
					...
					{
						std::unique_lock<std::shared_mutex> lock(currentEigrp->configs.configsMutex);
						currentEigrp->configs.maxPaths = static_cast<uint8_t>(std::stoi(commandStream[2]));
					}
				...
				else if (commandStream[0] == "timers")
				{
					if (commandStream[1] == "active-time")
					{
						...
						{
							std::unique_lock<std::shared_mutex> lock(currentEigrp->configs.configsMutex);
							currentEigrp->configs.activeTime = static_cast<uint16_t>(std::stoi(commandStream[2]));
							currentEigrp->configs.stuckInActiveTime = static_cast<uint16_t>(std::stoi(commandStream[2]) / 2);
						}
				...
				else if (commandStream[0] == "traffic-share")
				{
					...
					{
						std::unique_lock<std::shared_mutex> lock(currentEigrp->configs.configsMutex);
						currentEigrp->configs.trafficShareMode = EigrpConfigs::TrafficShareMode::Balenced;
					}
					else if (commandStream[1] == "min")
					{
						std::unique_lock<std::shared_mutex> lock(currentEigrp->configs.configsMutex);
						currentEigrp->configs.trafficShareMode = EigrpConfigs::TrafficShareMode::Minimum;
					}
				}
			...
			else if (currentSubMode == "eigrp_named")
			{
				if (currentMode == mode.routing)
				{
					if (commandStream[0] == "address-family")
					{
						...
						if (commandStream[2] == "autonomous-system" || commandStream[3] == "autonomous-system")
						{
							...
							{
								if (currentEigrpNamed->ipv4->asNumber != routingProtocolID)
								{
									std::cout << "\nChanging from AS(" << currentEigrpNamed->ipv4->asNumber << ") to AS(" << routingProtocolID << ") is not allowed";
									return false;
								}
							...
							{
								if (currentEigrpNamed->ipv6->asNumber != routingProtocolID)
								{
									std::cout << "\nChanging from AS(" << currentEigrpNamed->ipv4->asNumber << ") to AS(" << routingProtocolID << ") is not allowed";
									return false;
								}
							...
							else
							{
								...
								if (af == AddressFamily::IPv4)
								{
									...
									{
										std::cout << "\n%" << " ERROR: AS(" << routingProtocolID << ") in use by classic router";
										return false;
									}
									changeMode(mode.addressFamily);
									configureAddressFamily(AddressFamily::IPv4);
								...
								else if (af == AddressFamily::IPv6)
								{
									...
									{
										std::cout << "\nChanging from AS(" << eigrpAs->ipv4->asNumber << ") to AS(" << routingProtocolID << ") is not allowed";
									}
									else
									{
										std::cout << "\n%" << " ERROR: AS(" << routingProtocolID << ") in use by classic router";
				...
				else if (currentMode == mode.addressFamily)
				{
					...
					else if (command == "exit-address-family" && currentMode == mode.addressFamily)
					{
						...
						{
							configureRoutingMode("eigrp_named", false);
						}
