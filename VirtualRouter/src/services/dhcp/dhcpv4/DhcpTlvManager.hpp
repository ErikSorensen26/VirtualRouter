...
namespace Protocol
{
    namespace Dhcp
    {
        ...
        {
            DhcpTLVManager(DhcpHeader& dhcp, size_t size, bool isAuth, uint8_t* requests = nullptr, size_t reqSize = 0)
                : auth(isAuth),
            ...
            bool file = false;
            bool sname = true;
            TLV8BufferManager tlv;
            TLV8BufferManager tlvFile;
            TLV8BufferManager tlvSname;

            uint8_t* requestList = nullptr;
            size_t requestSize = 0;
        };

        inline static bool appendTLV(Dhcp::DhcpTLVManager& tlv, uint8_t type, uint8_t size, const uint8_t* value)
        {
        ...
        inline static bool appendTLV(Dhcp::DhcpTLVManager& tlv, uint8_t type, uint32_t value)
        {
            auto zeroRequested = [&]() {
