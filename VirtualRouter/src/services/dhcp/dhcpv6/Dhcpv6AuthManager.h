...
namespace Protocol
{
    namespace Dhcpv6
    {
        ...
        public:
            ...
            std::optional<DelayedAuthInfo> addDelayedAuthOption(TLV16BufferManager& tlv, const ClientID& duid);
            void addDelayedAuthDigest(Dhcpv6Header& dhcp, TLV16BufferManager& tlv, DelayedAuthInfo& auth);
            bool validateDelayedAuth(Dhcpv6Header& dhcp, TLV16Option* opt, const ClientID& duid);

            // RKAP
            std::optional<__uint128_t> addRkapAuthOption(TLV16BufferManager& tlv, const ClientID& duid);
            bool validateRkapDigest(Dhcpv6Header& dhcp, TLV16Option* opt, const ClientID& duid, __uint128_t rkapKey);
        ...
        public:
            ...
            std::optional<DelayedAuthInfo> addDelayedAuthOption(TLV16BufferManager& tlv);
            void addDelayedAuthDigest(Dhcpv6Header& dhcp, TLV16BufferManager& tlv, DelayedAuthInfo& auth);
            bool validateDelayedAuth(Dhcpv6Header& dhcp, TLV16Option* opt);

            // RKAP
            const uint8_t* addRkapAuthOption(TLV16BufferManager& tlv, const uint8_t* key);
            void addRkapAuthDigest(Dhcpv6Header& dhcp, TLV16BufferManager& tlv, const uint8_t* key, uint8_t* digest);
            uint8_t* validateRkapDigest(Dhcpv6Header& dhcp, TLV16Option* opt);
