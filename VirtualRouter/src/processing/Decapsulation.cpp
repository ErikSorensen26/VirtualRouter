...
// Parses and processes the DHCPv6 header
bool Packet::decodeDhcpv6(ByteString &dhcpHeaders)
{
    Dhcpv6Header dhcp;
    if (!dhcp.decapsulate(dhcpHeaders)) return false;
    ...
    if (print)
    {
        ...
        Logger::getInstance().info() << "DHCPv6 Frame:" << std::endl;
        Logger::getInstance().info() << "Message Type: " << dhcp.type.toHex() << std::endl;
        Logger::getInstance().info() << "Transaction ID: " << dhcp.transactionID.toHex() << std::endl;
