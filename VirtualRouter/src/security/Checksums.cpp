...
// Predefined CRC32 table (standard polynomial 0xEDB88320) for fast calculation.
...
// Calculates the CRC32 checksum of the given data and writes it to out, little-endian.
bool checksum::crc32(uint8_t* out, const uint8_t* data, size_t dataSize)
{
    ...
    out[0] = static_cast<uint8_t>(crc >> 0);
...
void checksum::calculateChecksum(uint8_t* packet, size_t headerSize, size_t checksumStartIndex, size_t checksumSize, const uint8_t* pseudoHeader, size_t pseudoHeaderSize, bool swap)
{
    ...
    switch (checksumSize)
    {
        ...
        {
            uint8_t sum = 0;
            for (size_t i = 0; i < pseudoHeaderSize; ++i)
                sum += pseudoHeader[i];
            for (size_t i = 0; i < headerSize; ++i)
                if (i < checksumStartIndex || i >= checksumStartIndex + 1)
                    sum += packet[i];
            packet[checksumStartIndex] = sum;
            break;
        }
        case 2:
        {
            ...
            if (i < pseudoHeaderSize)
                sum += static_cast<uint16_t>(pseudoHeader[i] << 8);
            i = 0;
        ...
        case 4:
        {
            ...
            {
                uint32_t last = 0;
                for (size_t b = 0; b < pseudoHeaderSize - i; ++b)
                    last = (last << 8) | pseudoHeader[i + b];
                last <<= (4 - (pseudoHeaderSize - i)) * 8;
                sum += last;
            }
            i = 0;
            ...
            {
                uint32_t last = 0;
                for (size_t b = 0; b < headerSize - i; ++b)
                    last = (last << 8) | packet[i + b];
                last <<= (4 - (headerSize - i)) * 8;
                sum += last;
            }
            while (sum >> 32)
            ...
            if (swap)
            {
                packet[checksumStartIndex + 0] = (checksum >> 0) & 0xFF;
                packet[checksumStartIndex + 1] = (checksum >> 8) & 0xFF;
                packet[checksumStartIndex + 2] = (checksum >> 16) & 0xFF;
                packet[checksumStartIndex + 3] = (checksum >> 24) & 0xFF;
            }
            else
            {
