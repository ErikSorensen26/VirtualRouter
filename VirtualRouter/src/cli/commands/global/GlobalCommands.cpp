...
bool Global_Exit_Handler(GLOBAL_PARAMS)
{
    ...
    ctx.terminal.changeModeConfig(new Cli::PrivilegedExecContext(ctx));
    return true;
}
...
bool Global_Interface_Handler(GLOBAL_PARAMS)
{
    ...
    ctx.terminal.configureInterfaceMode(type);
    ctx.terminal.changeModeConfig(new InterfaceContext(ctx, *ctx.global.getInterface(key)));
    return true;
}
...
#define ROUTER_CONFIG \
    ...
    { \
        ID = args[1]; \
        ctx.terminal.routingProtocolID = std::stoi(args[1]); \
    }
...
bool Global_RouterEIGRP_Handler(GLOBAL_PARAMS)
{
    ...
    if (Functions::isNumber(ID))
    {
        ...
        if (!ctx.negate)
        {
            ...
            if (!as->ipv4)
            {
                as->ipv4 = new Eigrp::Eigrp(ctx.terminal.routingProtocolID, AddressFamily::IPv4, ctx.global.getRoutingInstance("default"));
            }
            ctx.terminal.configureRoutingMode("eigrp_classic");
            ctx.terminal.changeModeConfig(new Cli::EigrpContext(ctx, as->ipv4, nullptr));
        }
        else
        {
            if (as)
    ...
    else
    {
        if (!ctx.negate)
        {
            ...
            ctx.terminal.configureRoutingMode("eigrp_named");
            ctx.terminal.changeModeConfig(new Cli::EigrpContext(ctx, nullptr, ctx.vrf.getEigrpNamed(ID)));
        }
        else
        {
