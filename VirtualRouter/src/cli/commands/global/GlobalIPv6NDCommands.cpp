...
{
    uint16_t value = ctx.negate ? 600 : std::stoi(args[0]);
...
bool GlobalIPv6ND_CacheIntLimit_Handler(GLOBAL_PARAMS)
{
    ...
    {
        setLog = true;
        log = ctx.negate ? 0 : std::stoi(args[1]);
...
{
    uint16_t time = ctx.negate ? 1000 : std::stoi(args[0]);
    for (const auto& [_, iface] : ctx.global.getInterfaceList())
    {
...
{
    ctx.global.configs.ndp.nsfConvergenceTime.store(ctx.negate ? 180 : std::stoi(args[0]), std::memory_order_release);
    return true;
}

bool GlobalIPv6ND_NSFDADSuppress_Handler(GLOBAL_PARAMS)
{
    ctx.global.configs.ndp.nsfDadSupressionTime.store(ctx.negate ? 180 : std::stoi(args[0]), std::memory_order_release);
    return true;
}

bool GlobalIPv6ND_NSFThrottle_Handler(GLOBAL_PARAMS)
{
    ctx.global.configs.ndp.nsfThrottleResolutions.store(ctx.negate ? 1000 : std::stoi(args[0]), std::memory_order_release);
    return true;
}

bool GlobalIPv6ND_NudLimit_Handler(GLOBAL_PARAMS)
{
    ctx.global.configs.ndp.nudLimit.store(ctx.negate ? 5 : std::stoi(args[0]), std::memory_order_release);
    if (args.size() > 1)
    {
        ctx.global.configs.ndp.nudRefreshPeriod.store(ctx.negate ? 5 : std::stoi(args[2]), std::memory_order_release);
    }
    return true;
}
...
{
    uint16_t value = ctx.negate ? 30000 : std::stoi(args[0]);
...
{
    ctx.global.configs.ndp.resolutionLimit.store(ctx.negate ? 512 : std::stoi(args[0]), std::memory_order_release);
    return true;
}
