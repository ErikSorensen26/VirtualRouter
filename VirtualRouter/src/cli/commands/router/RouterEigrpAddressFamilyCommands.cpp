...
bool RouterEigrpAddressFamily_AfInterface_Handler(EIGRP_PARAMS)
{
    ...
    if (!ctx.negate)
    {
        ...
        ctx.terminal.changeMode(CliMode::RouterAddressFamilyInterface);
        ctx.terminal.configureAddressFamily(ctx.currentEigrp->getAF());
    }
    else
    {
        if (intIt != ifaceMgr.eigrpInterfaceConfigList.end())
...
bool RouterEigrpAddressFamily_Topology_Handler(EIGRP_PARAMS)
{
    if (!ctx.negate)
    {
        if (args[0] == "base")
        {
            ctx.terminal.changeMode(CliMode::RouterAddressFamilyTopology);
            ctx.terminal.configureAddressFamily(ctx.currentEigrp->getAF());
        }
        else
        {
