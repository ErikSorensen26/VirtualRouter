...
bool EigrpPacketBuilder::appendParameterTLV(packet::TLV16BufferManager& tlv, EigrpInterface& iface)
{
    ...
    {
        std::memset(val, 0, 6);
        utils::writeU16(val + 6, iface.configs.reg.get<config::EigrpInterface::HOLD_TIME>().load());
    ...
    else
    {
        ...
        TLVBuilder::calculateParameters(val, k, iface.configs.reg.get<config::EigrpInterface::HOLD_TIME>().load());
    }
