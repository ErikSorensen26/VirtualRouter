...
void InterfaceTimers::restartDampeningResetTimer()
{
    ...
    suppressedUntil = std::chrono::steady_clock::now() + std::chrono::seconds(iface.getConfigs().get<config::Eigrp::MAXIMUM_PREFIX_RESET_TIME>().load()),
...
void InterfaceTimers::restartDampeningRestartTimer()
{
    ...
    dampeningRestartId.store(ref.postAfter(
        std::chrono::steady_clock::now() + std::chrono::seconds(iface.getConfigs().get<config::Eigrp::MAXIMUM_PREFIX_RESTART>().load()),
