...
json getCommandTree(CLIENT_TYPE& telnet, const std::string& prefix, int depth = 0, int maxDepth = 30, std::vector<std::unordered_set<std::string>>* ancestorResponses = nullptr) {
    // Stop if we reach the maximum recursion depth.
    if (depth >= maxDepth)
        return json::array();

    std::vector<std::unordered_set<std::string>> ownAncestorResponses;
    if (!ancestorResponses)
        ancestorResponses = &ownAncestorResponses;
    ...
    // Stop if this node's response set matches an ancestor's in the current
    // chain (cycle: the menu has looped back to an identical option list).
    std::unordered_set<std::string> lineSet(lines.begin(), lines.end());
    for (const auto& seen : *ancestorResponses)
    {
        if (seen == lineSet)
            return json::array();
    }
    ancestorResponses->push_back(lineSet);
    ...
    for (const std::string& line : lines) {
        ...
        if (!isLine)
        {
            subcommands = getCommandTree(telnet, newPrefix, depth + 1, maxDepth, ancestorResponses);
        }
