...
protected:
    ...
    void SetUp() override 
    {
        eigrpList[1] = new EigrpAutonomousSystem();
        eigrpList[1]->ipv4 = eigrpInstance;
        ...
        eigrpInstance->eigrpInterfaceList[0] = eigrpInterface;
    }
    ...
    std::unordered_map<uint8_t, EigrpInterface*>& getInterfaceList() { return eigrpInstance->eigrpInterfaceList; }
    ...
    void setIPv6(const ByteString& ip, uint8_t mask, Interface* iface = nullptr) 
    {
        if (iface)
            ...
            mockInterface->setIPv6(ip, mask, false);
    }
    ...
    std::optional<EigrpConfigs::NeighborInfo*> getNeighbor(const ByteString& ip, EigrpInterface* intf = nullptr) 
    {
...
TEST_F(EigrpTest, AuthTLV_MD5_Correct) 
{
    ...
    ASSERT_TRUE(optNeighbor.has_value());
    auto neighbor = optNeighbor.value();
...
TEST_F(EigrpTest, NeighborState_DOWN_To_EXSTART) 
{
    ...
    auto neighbor = getNeighbor(neighborIp).value();
    ...
    eigrpInterface->processHello(neighbor, hello, neighborIp);
...
TEST_F(EigrpTest, Neighbor_Restart_Resets_State) 
{
    ...
    neighbor->sequenceList[100].init = true;
    ...
    ASSERT_TRUE(neighbor->sequenceList.empty());
}
...
TEST_F(EigrpTest, HoldTimer_Expires_Marks_Neighbor_Down) 
{
    ...
    ASSERT_FALSE(getNeighbor(neighborIp).has_value());
}
...
TEST_F(EigrpTest, MultipleNeighbors_Independent_States) 
{
    ...
    auto neighbor1 = getNeighbor(neighborIp1).value();
    auto neighbor2 = getNeighbor(neighborIp2).value();
    ...
    eigrpInterface->processHello(neighbor1, hello1, neighborIp1);
...
TEST_F(EigrpTest, External_Route_TLV_Format) 
{
    ...
    ByteString extTLV = eigrpInterface->encodeExternalRouteOption(&externalRoute, false);
...
TEST_F(EigrpTest, Internal_Route_TLV_Format) 
{
    ...
    ByteString intTLV = eigrpInterface->encodeRouteOption(&internalRoute, false);
...
TEST_F(EigrpTest, IPv6_Config_Persistence) 
{
    ...
    ASSERT_EQ(getIpInfo().ipv6.ipAddress, ByteString("\xc0\xa8\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x01", 16));
}

// Test MAC_Resolution_IPv4
TEST_F(EigrpTest, MAC_Resolution_IPv4) 
{
    // Simulate ARP resolution on an IPv4 neighbor.
    auto mac = RoutingTable::getInstance().ArpLookup(ByteString("\xc0\xa8\x01\x02", 4));
    if (mac.has_value()) {
        eigrpInterface->resolveMacAddress(getNeighbor(ByteString("\xc0\xa8\x01\x02", 4)).value());
    }
    SUCCEED();
}

// Test MAC_Resolution_IPv6
TEST_F(EigrpTest, MAC_Resolution_IPv6) 
{
    // For IPv6, simply call resolveMacAddress (even if not fully implemented).
    eigrpInterface->resolveMacAddress(getNeighbor(ByteString("\x20\x01\x0D\xB8\x00\x00\x00\x02", 8)).value());
    SUCCEED();
}
...
TEST_F(EigrpTest, Connected_Route_Addition_On_Interface_Up) 
{
    ...
    ASSERT_TRUE(RoutingTable::getInstance().getEigrpRoute(ByteString("\xc0\xa8\x01\x00", 4),
                                                            24, AddressFamily::IPv4, asNumber).has_value());
}
...
TEST_F(EigrpTest, Connected_Route_Removal_On_Interface_Down) 
{
    ...
    clearNetworks();
    eigrpInstance->updateInterfaceList();
    ASSERT_FALSE(RoutingTable::getInstance().getEigrpRoute(ByteString("\xc0\xa8\x01\x00", 4),
                                                             24, AddressFamily::IPv4, asNumber).has_value());
}
...
TEST_F(EigrpTest, Multiple_Connected_Routes_From_Different_Interfaces) 
{
    ...
    MockInterface* extraIface1 = new MockInterface();
    extraIface1->Get()->id = 1;
    extraIface1->Get()->interfaceType = InterfaceType::GIGABIT_ETHERNET;
    ...
    interfaceList[InterfaceType::GIGABIT_ETHERNET][1] = extraIface1;

    MockInterface* extraIface2 = new MockInterface();
    extraIface2->Get()->id = 2;
    extraIface2->Get()->interfaceType = InterfaceType::GIGABIT_ETHERNET;
    extraIface2->blockEnqueues();
    extraIface2->enableIPs();
    extraIface2->enableShutdown();
    setIPv4(ByteString("\x0A\x00\x20\x03", 4), 24, extraIface2);
    interfaceList[InterfaceType::GIGABIT_ETHERNET][2] = extraIface2;
    ...
    eigrpInstance->updateInterfaceList();
    auto allRoutes = RoutingTable::getInstance().getAllEigrpRoutes(AddressFamily::IPv4, asNumber);
...
TEST_F(EigrpTest, RoutingTable_Metric_Update_On_Best_Route_Change) 
{
    ...
    RoutingTable::getInstance().addEigrp(route1, AddressFamily::IPv4, asNumber);
    ...
    RoutingTable::getInstance().addEigrp(route2, AddressFamily::IPv4, asNumber);
    
    auto r = RoutingTable::getInstance().getEigrpRoute(ByteString("\xc0\xa8\x02\x00", 4),
                                                        24, AddressFamily::IPv4, asNumber);
    ASSERT_TRUE(r.has_value());
    ASSERT_EQ(r.value()->feasibleDistance, 50);
    
    delete route1; delete route2;
}
...
TEST_F(EigrpTest, RoutingTable_All_Connected_Routes_Count) 
{
    ...
    network.mask = ByteString("\x00\x00\xff\xff", 4);
    eigrpInstance->addNetwork(network);
    auto routes = RoutingTable::getInstance().getAllConnectedEigrpRoutes(AddressFamily::IPv4, asNumber);
...
TEST_F(EigrpTest, StubMode_Enabled_Allows_Only_Permitted_Routes) 
{
    ...
    eigrpInstance->setStub(true, true, false, false, false);
    RoutingTable::Eigrp externalRoute;
    externalRoute.network = ByteString("\xc0\xa8\x02\x00", 4);
    externalRoute.mask = 24;
    externalRoute.routeType = "external";
...
TEST_F(EigrpTest, StubMode_Advertise_Static_Only) 
{
    ...
    addNeighbor(ByteString("\xc0\xa8\x00\x02", 4));
    eigrpInstance->setStub(true, false, true, false, false);
    RoutingTable::Eigrp staticRoute;
    staticRoute.network = ByteString("\xc0\xa8\x04\x00", 4);
    staticRoute.mask = 24;
    staticRoute.routeType = "static";
...
TEST_F(EigrpTest, StubMode_Advertise_Summary_Only) 
{
    ...
    addNeighbor(ByteString("\xc0\xa8\x00\x02", 4));
    eigrpInstance->setStub(true, false, false, true, false);
...
TEST_F(EigrpTest, StubMode_Advertise_Redistributed_Only) 
{
    ...
    addNeighbor(ByteString("\xc0\xa8\x00\x02", 4));
    eigrpInstance->setStub(true, false, false, false, true);
    RoutingTable::Eigrp externalRoute;
...
#pragma endregion
#pragma region DefaultRoute

// Test: DefaultRoute_Addition_Metric_Calculation
TEST_F(EigrpTest, DefaultRoute_Addition_Metric_Calculation) 
{
    // Verify that a default route is added with the correct metric.
...
// Test: DefaultRoute_Only_Added_If_Interface_Active
TEST_F(EigrpTest, DefaultRoute_Only_Added_If_Interface_Active) 
{
    // If the interface is down, default route should not be added.
    eigrpInstance->addNetwork(EigrpConfigs::Network{ByteString("\xc0\xa8\x00\x00", 4), ByteString("\x00\x00\xff\xff", 4)});
    mockInterface->Shutdown(true);
    eigrpInstance->addDefaultRoute();
    auto defaultRoute = RoutingTable::getInstance().getEigrpRoute(eigrpInstance->getConfigs()->defaultNetwork,
...
// Test: DefaultRoute_Removal_Cleans_Routing_Table
TEST_F(EigrpTest, DefaultRoute_Removal_Cleans_Routing_Table) 
{
    // Verify that removing the default route clears it from the routing table.
    eigrpInstance->addNetwork(EigrpConfigs::Network{ByteString("\xc0\xa8\x00\x00", 4), ByteString("\x00\x00\xff\xff", 4)});
    eigrpInstance->addDefaultRoute();
...
// Test: DefaultRoute_ReAdd_After_Interface_Recovery
TEST_F(EigrpTest, DefaultRoute_ReAdd_After_Interface_Recovery) 
{
    // Verify that a default route is re-advertised when an interface recovers.
    eigrpInstance->addNetwork(EigrpConfigs::Network{ByteString("\xc0\xa8\x00\x00", 4), ByteString("\x00\x00\xff\xff", 4)});
    mockInterface->Shutdown(true);
    eigrpInstance->updateInterfaceList();
    ASSERT_TRUE(getInterfaceList().empty());
...
TEST_F(EigrpTest, ActiveQuery_Clear_After_Neighbor_Response) 
{
    ...
    eigrpInterface->processReply(getNeighbor(pendingNeighbor).value(), pendingNeighbor, EigrpHeader());
    eigrpInstance->outstandingReplies.erase(queryKey);
...
// Test: GracefulRestart_Sends_Hello_To_All_Neighbors
TEST_F(EigrpTest, GracefulRestart_Sends_Hello_To_All_Neighbors) 
{
    // Verify that hello packets are re-sent during graceful restart.
    addNeighbor(ByteString("\x0A\x00\x00\x03", 4), eigrpInterface);
    EXPECT_CALL(*mockInterface, enqueuePacket(::testing::_, ::testing::_))
        .Times(::testing::AtLeast(1));
...
TEST_F(EigrpTest, IPv6_HelloPacket_Construction) 
{
    ...
    auto* ipv6Eigrp = new Eigrp(asNumber, AddressFamily::IPv6);
    ipv6Eigrp->initializeEigrp();
    MockInterface* ipv6Interface = new MockInterface();
    ...
    interfaceList[InterfaceType::GIGABIT_ETHERNET][10] = ipv6Interface;
    EigrpInterface* ipv6Int = new EigrpInterface(*ipv6Eigrp, ipv6Interface);
    ipv6Eigrp->eigrpInterfaceList[10] = ipv6Int;
    
    EigrpHeader helloPacket;
    ByteString neighborIp("\x20\x01\x0D\xB8\x00\x00\x00\x02", 16);
    addNeighbor(neighborIp, ipv6Int);
    ipv6Eigrp->eigrpHello(helloPacket, ipv6Int, getNeighbor(neighborIp, ipv6Int).value(), neighborIp, 800, false, false);
...
// Test: IPv6_RouteSummarization
TEST_F(EigrpTest, IPv6_RouteSummarization) 
{
    // Verify that adding an IPv6 summary route creates the proper routing entry.
    delete eigrpInstance;
    ...
    auto routeOpt = RoutingTable::getInstance().getEigrpRoute(ByteString("\x20\x01\x0D\xB8\x00\x00\x00\x00", 8),
                                                              64, AddressFamily::IPv6, asNumber);
    ASSERT_TRUE(routeOpt.has_value());
}

// Test: IPv6_Neighbor_Discovery_And_MAC_Resolution
TEST_F(EigrpTest, IPv6_Neighbor_Discovery_And_MAC_Resolution) 
{
    // For IPv6, simply call resolveMacAddress and verify no crash.
    eigrpInterface->resolveMacAddress(getNeighbor(ByteString("\x20\x01\x0D\xB8\x00\x00\x00\x03", 8)).value());
    SUCCEED();
}
...
TEST_F(EigrpTest, IPv6_PacketSerialization_Correct_Length) 
{
    ...
    addNeighbor(neighborIp, ipv6Int);
    ipv6Eigrp->eigrpHello(helloPacket, ipv6Int, getNeighbor(neighborIp, ipv6Int).value(), neighborIp, 900, false, false);
    ByteString serialized = eigrpInterface->serializeEigrpHeader(helloPacket, false);
...
TEST_F(EigrpTest, MultiInterface_Massive_Concurrent_Updates) 
{
    ...
    MockInterface* iface1 = new MockInterface();
    MockInterface* iface2 = new MockInterface();
    ...
    getInterfaceList()[1] = int1;
    getInterfaceList()[2] = int2;
    ...
    for (int i = 0; i < 300; i++) 
    {
        ...
        int1->updateRoutingTable(getNeighbor(ByteString("\x0A\x00\x00\x01", 4), int1).value(), ByteString("\x0A\x00\x00\x01", 4), {&route}, false);
    }
    auto routes = RoutingTable::getInstance().getAllEigrpRoutes(AddressFamily::IPv4, asNumber);
    ASSERT_GE(routes.size(), 1);
    eigrpInstance->shutdown();
...
TEST_F(EigrpTest, Rapid_Neighbor_AddRemove_Convergence) 
{
    ...
    for (int i = 0; i < 50; i++) {
        ...
        if(neighbor.has_value())
            eigrpInterface->handleHoldTimeExpire(neighbor.value(), ip);
    }
    SUCCEED();
}
...
TEST_F(EigrpTest, MultiInterface_Massive_Concurrent_Updates_Extended) 
{
    ...
    for (int i = 0; i < 300; i++) {
        ...
        route.routeType = "internal";
        int1->updateRoutingTable(getNeighbor(neighbor1, int1).value(), neighbor1, {&route}, false);
    }
    auto routes = RoutingTable::getInstance().getAllEigrpRoutes(AddressFamily::IPv4, asNumber);
    ASSERT_GE(routes.size(), 1);
    eigrpInstance->shutdown();
...
TEST_F(EigrpTest, MultiInterface_Coordinated_RoutingUpdates_Extended) 
{
    ...
    int1->updateRoutingTable(getNeighbor(neighborIp, int1).value(), neighborIp, {&route1, &route2}, false);
    auto routes = RoutingTable::getInstance().getAllEigrpRoutes(AddressFamily::IPv4, asNumber);
