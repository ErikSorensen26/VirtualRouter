...
protected:
    ...
    void SetUp() override
    {
        ...
        auto* config = new DhcpNetworkConfig();

        config->interface = iface;
        config->network = ByteString("\x20\x01\x0D\xB8\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16);
        config->subnetPrefix = 56;
    ...
    std::unordered_map<ByteString, DhcpNetwork*>& getNetworks() { return server->dhcpNetworks; }
    Protocol::DhcpNetwork* net = nullptr;
    ...
    Protocol::Dhcpv6::AuthConfig enableAuth(const ByteString& key)
    {
        server->authenticationEnabled.store(true, std::memory_order_release);
        server->authConfig.sharedSecret = key;
        return server->authConfig;
    }

    std::map<Dhcp::TimerType, std::vector<Dhcp::TrackedTimer>>& getActiveTimers() { return server->activeTimers; }
    Dhcpv6Header::Option buildAuthOption() {return server->buildAuthenticationOption();}
    std::map<ByteString, Dhcpv6::ReconfigureState>& getReconfigs() {return server->activeReconfigs;}
    void sendReconfigure() {server->sendReconfigure();}
...
TEST_F(Dhcpv6ServerTest, Solicit_ValidIA_NA_NoRapidCommit_AllocatesTempLease) 
{
    ...
    server->handleDhcpPacket(packet, iface);
...
TEST_F(Dhcpv6ServerTest, Solicit_ValidIA_NA_WithRapidCommit_CommitsLease) 
{
    ...
    server->handleDhcpPacket(pkt, iface);
...
*/
#pragma endregion
#pragma region EchoRequest
/*

// Echo-Request: With valid client ID returns echo reply
TEST_F(Dhcpv6ServerTest, EchoRequest_Valid_ReturnsEchoReply)
{
    Dhcpv6Header echo = makePacket(Variable::Dhcpv6::Type::echoRequest, false);

    PacketInfo pkt;
    pkt.Layer5.push_back(echo);

    expectPacket(Variable::Dhcpv6::Type::echoReply);
    server->handleDhcpPacket(pkt, iface);
}

// Echo-Request: Missing client DUID should be ignored
TEST_F(Dhcpv6ServerTest, EchoRequest_MissingDUID_Ignored)
{
    Dhcpv6Header echo;
    echo.type = Variable::Dhcpv6::Type::echoRequest;
    echo.transactionID = makeTransID();
...
// Echo-Request: If server ID is present, ignore request
TEST_F(Dhcpv6ServerTest, EchoRequest_WithServerID_Ignored)
{
    Dhcpv6Header echo = makePacket(Variable::Dhcpv6::Type::echoRequest);

    PacketInfo pkt;
    pkt.Layer5.push_back(echo);

    expectPacket(Variable::Dhcpv6::Type::echoReply, 0);
    expectNoPacket();
    server->handleDhcpPacket(pkt, iface);
}

// Echo-Request: Invalid authentication should be ignored
TEST_F(Dhcpv6ServerTest, EchoRequest_AuthInvalid_Ignored)
{
    Dhcpv6Header echo = makePacket(Variable::Dhcpv6::Type::echoRequest, false);
...
TEST_F(Dhcpv6ServerTest, Auth_ValidMAC_Passes)
{
    ...
    auto authConfig = enableAuth(secret);
    ByteString duid = getClientDUID();

    solicit.options.push_back(buildAuthOption());
...
TEST_F(Dhcpv6ServerTest, Auth_ReplayCounterTooLow_Fails)
{
    ...
    PacketInfo pkt;
    Dhcpv6Header::Option auth = buildAuthOption();
    first.options.push_back(auth);
    pkt.Layer5.push_back(first);
    expectPacket(Variable::Dhcpv6::Type::advertise);
    server->handleDhcpPacket(pkt, iface);

    // Now reuse the same reply counter, which is too low
    solicit.options.push_back(auth);
    PacketInfo pkt2;
    pkt2.Layer5.push_back(solicit);
    expectNoPacket();
    server->handleDhcpPacket(pkt2, iface);
}
...
TEST_F(Dhcpv6ServerTest, Auth_BuildAuthOption_ProducesValidOption)
{
    ...
    Dhcpv6Header::Option authOpt = buildAuthOption();

    ASSERT_EQ(authOpt.option, Variable::Dhcpv6::Options::auth);
    ASSERT_GE(authOpt.length.size(), 2);
    ASSERT_GE(authOpt.value.size(), 25);

    // Protocol = 0x0001 (delayed), Algorithm = 0x0001 (HMAC-MD5), RDM = 0x00
    EXPECT_EQ(authOpt.value[0], 0x00);
    EXPECT_EQ(authOpt.value[1], 0x01);
    EXPECT_EQ(authOpt.value[2], 0x00);
    EXPECT_EQ(authOpt.value[3], 0x01);
    EXPECT_EQ(authOpt.value[4], 0x00);

    // MAC is the last 16 bytes
    ByteString mac = std::string(authOpt.value.end() - 16, authOpt.value.end());
...
TEST_F(Dhcpv6ServerTest, Auth_ReplyIncludesAuthOption)
{
    ...
    enableAuth("testKey");
    request.options.push_back(buildAuthOption());
...
// Timer: Reconfigure timeout should trigger retries up to max retry cost
TEST_F(Dhcpv6ServerTest, Timer_ReconfigureTimeout_Retries)
{
    ByteString iaid = generateIAID();
    ByteString id = getClientDUID() + iaid;
    ByteString ip = net->pool->allocateTempIP(&id, true);
    ...
    getConfigs().recMaxRc = 3;
    getConfigs().recTimeout = 1;

    // Create cv to wait for replies
    std::mutex reconfigMutex;
    std::condition_variable cv;
    size_t retryCount = 0;

    // Expect 3 retries
    expectPacket(Variable::Dhcpv6::Type::reconfigure, 3, [&](const Dhcpv6Header& pkt) {
        for (const auto& opt : pkt.options)
        {
            if (opt.option == Variable::Dhcpv6::Options::reconfigureMessage)
            {
                ++retryCount;
                if (retryCount == 3)
                {
    ...
    // Wait for 3 reconfigs to send
    {
        std::unique_lock<std::mutex> lock(reconfigMutex);
        EXPECT_TRUE(cv.wait_for(lock, std::chrono::seconds(5), [&]() { return retryCount == 3; }));
        std::this_thread::sleep_for(std::chrono::seconds(1));
    }

    // Make sure reconfig was removed after timed out
    ASSERT_TRUE(getReconfigs().empty());
}
...
TEST_F(Dhcpv6ServerTest, Scenario_ReleaseThenSolicit_ReusesAddress)
{
    ...
    expectPacket(Variable::Dhcpv6::Type::reply);
    server->handleDhcpPacket(releasePkt, iface);
    ...
    server->handleDhcpPacket(solicitPkt, iface);
}
...
// Scenario: Echo request after successful confirm should succeed
TEST_F(Dhcpv6ServerTest, Scenario_EchoRequestAfterConfirm_Validates)
{
    ByteString iaid = generateIAID();
    ByteString id = getClientDUID() + iaid;
    ByteString ip = net->pool->allocateTempIP(&id, true);
    ...
    Dhcpv6Header confirm = makePacket(Variable::Dhcpv6::Type::confirm, false);
    addIANA(iaid, confirm, {ip});
    PacketInfo confirmPkt;
    confirmPkt.Layer5.push_back(confirm);

    expectPacket(Variable::Dhcpv6::Type::reply);
    server->handleDhcpPacket(confirmPkt, iface);

    Dhcpv6Header echo = makePacket(Variable::Dhcpv6::Type::echoRequest, false);
    PacketInfo echoPkt;
    echoPkt.Layer5.push_back(echo);
    expectPacket(Variable::Dhcpv6::Type::echoReply);
    server->handleDhcpPacket(echoPkt, iface);
}
